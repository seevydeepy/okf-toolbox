...
7. Update shallowly: usually the matched `<docs-root>/okf/<solution>/routing.md`, `solution.md`, `log.md`, and sometimes the discovered `solutions.manifest.json`. If the repo still has a legacy context map or compatibility wiki pages, keep them aligned only where the change affects routing or claims.
